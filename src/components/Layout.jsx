...
import { useAuth } from '../contexts/AuthContext';
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col w-full max-w-md mx-auto shadow-2xl relative">
      ...
      <footer className="bg-white border-t border-gray-200 fixed bottom-0 w-full max-w-md flex justify-around p-3 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] z-20">
        <div onClick={handleHomeClick} className={`flex flex-col items-center cursor-pointer ${!isProfile ? 'text-indigo-600' : 'text-gray-400 hover:text-indigo-400'}`}>
           <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
          </svg>
          <span className="text-xs font-medium mt-1">首頁</span>
        </div>
        {userRole !== 'admin' && (
          <div onClick={handleProfileClick} className={`flex flex-col items-center cursor-pointer ${isProfile ? 'text-indigo-600' : 'text-gray-400 hover:text-indigo-400'}`}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
            <span className="text-xs font-medium mt-1">基本資料</span>
          </div>
        )}
      </footer>
    </div>
