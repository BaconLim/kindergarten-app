      ...
      {filteredBooks.map(book => (
        <div key={book.id} className="bg-white p-4 rounded-xl shadow-sm border border-yellow-200 space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleDelete(book.id)} className="px-3 py-2 bg-red-50 text-red-600 rounded-lg text-xs">刪除</button>
            {editingBookId === book.id ? (
            ...
            <button onClick={() => handlePublish(book.id)} className="flex-1 py-2 bg-green-500 text-white rounded-lg text-xs font-bold">單篇發布</button>
          </div>
        </div>
      ))}
      
      {filteredBooks.length > 0 && (
        <button onClick={() => handlePublishAll(filteredBooks)} className="w-full py-3 bg-green-600 text-white rounded-xl font-bold shadow-lg">
